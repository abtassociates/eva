...
output$all_q_avg <- renderDT({
  ...
  data <- hh_avg_q() 
  labels <- data$label
  data <- data %>% select(-label, -starts_with("PIT "), -starts_with("Total ")) %>% t()
  colnames(data) <- labels
  datatable( # return table
    data ,
    #rownames = data$label,
