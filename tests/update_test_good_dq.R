...
split_rows <- select_random_rows(original_data$Exit$Destination == 435)
original_data$Exit <- original_data$Exit %>%
  fmutate(
    rn = seq_row(.),
...
# CLS Subsidy Type -------------------------------------------------------------
split_rows <- select_random_rows(original_data$CurrentLivingSituation$CurrentLivingSituation == 435)
original_data$CurrentLivingSituation <- original_data$CurrentLivingSituation %>%
  fmutate(
    rn = seq_row(.),
...
split_rows <- select_random_rows(original_data$Enrollment$LivingSituation == 435)
...
date_shift <- -2
overlaps_temp <- original_data$HMISParticipation %>%
...
original_data$HMISParticipation <- bind_rows(overlaps_temp, original_data$HMISParticipation)

# RRH-SO projects with active inventory (checkID = 132) ------------------------
# this selects a random project with inventory and projecttype = 13
random_project <- original_data$Inventory %>% 
...
original_data$Project <- original_data$Project %>%
  fmutate(
    RRHSubType = fifelse(ProjectID == random_project, 1, RRHSubType) #,
    # RRHSOActivePeriod =
    #   interval(OperatingStartDate,
    #     coalesce(OperatingEndDate, original_data$Export$ExportEndDate))
  )
...
original_data$Organization <- rbind(
  original_data$Organization[1,], 
...
original_data[tables_to_filter] <- lapply(
  original_data[tables_to_filter],
...
original_data$Project[5, "ProjectName"] <- "Organization G - [Brackets]"
original_data$Project[7, "ProjectName"] <- "Organization S - ‘smart quote’"
original_data$Project[8, "ProjectName"] <- "Organization M - “Double smart quote”"
# per mille sign but Organization will be left as utf-8 encoded
original_data$Organization[2, "OrganizationName"] <- paste0(
  original_data$Organization[2, "OrganizationName"],
  "‰"
)

# missing address, for VSP and CoC, as well as CoCCode and GeographyType
original_data$Organization[11, "VictimServiceProvider"] <- 1 # org 55
# Non-VSP project (Org 49) and a project with Org 55
original_data$ProjectCoC[c(1, 28), c("Address1", "CoCCode", "GeographyType", "ZIP")] <- NA
...
coc_project <- original_data$Project %>% 
      fsubset(ContinuumProject == 1) 
coc_project <- coc_project$ProjectID[fnrow(coc_project)]

original_data$ProjectCoC <- original_data$ProjectCoC %>%
...
XInventory <- original_data$Inventory %>%
