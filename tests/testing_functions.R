...
main_test_script <- function(test_script_name, test_dataset) {
  test_that(paste0("{shinytest2} recording: ",test_script_name), {
    ...
    app$wait_for_value(input="fileStructureAnalysis") #wait until whole application is idle for 500ms
    ...
    app$wait_for_value() #wait until whole application is idle for 500ms
