...
Client <- Client %>%
  mutate(
    DOB <- format(DOB, "%d-%m-%Y"), # Incorrect Date Format, High-Priority
    DateUpdated <- format(DateUpdated, "%d-%m-%Y"), # Incorrect Date Format, Error
    ExtraCol <- 1 # extra column
  ) %>%
  select(
    -DateCreated,  # missing column, error
    -BlackAfAmerican, # missing column, high-priority
  ) %>% 
  rename(Transgenderr = Transgender) # misspelled column
...
cols <- c("VeteranStatus", "RaceNosne", "AmIndAKNative", "Asian", "BlackAfAmerican", 
...
# count rows with an invalid value 
...
# get a dataframe with one row per field containing at least one invalid value
