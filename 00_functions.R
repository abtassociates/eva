...
zip_initially_valid <- function () {
  ...
  if(grepl("/", zipContents$Name[1])) {
    ...
    write(paste(session$token, ": Unsuccessful - file was mistructured"), "www/metadata/upload_metadata.txt", append=TRUE)
    ...
    "
    write(paste(session$token, ": Unsuccessful - wrong dataset"), "www/metadata/upload_metadata.txt", append=TRUE)
  } 
  else if(!is_hashed()) {
    ...
    err_msg = "You have uploaded an unhashed version of the HMIS CSV Export. If you
          are not sure how to run the hashed HMIS CSV Export in your HMIS, please
          contact your HMIS vendor."
    write(paste(session$token, ": Unsuccessful - not hashed"), "www/metadata/upload_metadata.txt", append=TRUE)
  } else {
    return(TRUE)
  }
