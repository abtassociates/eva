...
build_demographic_heatmap <- function(plot_df, 
  ...
  # 4. Base Heatmap Plot
  g <- ggplot(plot_df, aes(x = .data[[x_var]], y = .data[[y_var]])) +
    geom_tile(
...
sys_comp_plot <- function(subtab = 'comp', methodology_type, selections, isExport = FALSE) {
  # 1. Run Data Preparation Pipeline
  prep <- prepare_crosstab_data(
    df = get_people_universe_filtered(), 
...
sys_comp_data_download <- function(file, type = 'syso') {
  ...
  selections <- if(type == 'syso') input$system_composition_selections else input$syse_phd_selections
  
  if(is.null(sys_comp_plot_df())) {
    sys_comp_plot(
      subtab = subtab,
      methodology_type = methodology_type,
      selections = selections,
...
limit_checkbox_selections <- function(input_id, selected_values) {
  shinyjs::runjs(str_glue("
    ...
    var numSelected = {length(selected_values)};
    $('input[name={input_id}]:not(\":checked\")').attr('disabled', numSelected == 2);
    var reSelected = \"{'All Races/Ethnicities' %in% selected_values || 'Grouped Races/Ethnicities' %in% selected_values}\";
    if(numSelected == 1)
      $('input[name={input_id}][value*=\"Races/Ethnicities\"]:not(\":checked\")').attr('disabled', reSelected == 'TRUE');
  "))
}


observeEvent(input$system_composition_selections, {
  limit_checkbox_selections("system_composition_selections", input$system_composition_selections)
}, ignoreNULL = FALSE)
...
output$sys_comp_summary_ui_chart <- renderPlot({
  ...
  build_demographic_heatmap(
    plot_df = plot_df,
    selections = input$system_composition_selections, # length 1
    metric_type = "count",
    palette_type = "purple",
    total_count_display = sys_total_count_display(nrow(comp_df)),
    isExport = FALSE
  )
...
sys_comp_ppt_download <- function(file, type = 'syso') {
  ...
  selections <- if(type == 'overview') input$system_composition_selections else input$syse_phd_selections
  ...
  selections_info <- if(type == 'overview') sys_comp_selections_info() else sys_phd_selections_info()
