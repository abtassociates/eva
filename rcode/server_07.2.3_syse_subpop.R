...
toggle_subpop_download_options <- function() {
  ...
  # Identify the checkbox input IDs for the subpop items
  # (Replace these with the actual IDs generated by your get_sys_export_id function)
  subpop_ppt_id  <- get_sys_export_id("syse", list(suffix = "subpop", ext = ".pptx"))
  subpop_xlsx_id <- get_sys_export_id("syse", list(suffix = "subpop", ext = ".xlsx"))
  
  # 1. Enable / Disable checkboxes
  shinyjs::toggleState(subpop_ppt_id, condition = has_subpops)
  shinyjs::toggleState(subpop_xlsx_id, condition = has_subpops)
  
  # 2. If disabled, uncheck them so they are not included in the export
  if (!has_subpops) {
    updateCheckboxInput(session, subpop_ppt_id, value = FALSE)
    updateCheckboxInput(session, subpop_xlsx_id, value = FALSE)
  }
  
  # 3. Show info icon when disabled, hide when enabled
  shinyjs::toggle("syse_subpop_info_icon", condition = !has_subpops)
}
