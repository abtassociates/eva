...
new_way_dt <- data.table::fread(here("metadata-analysis/metadata/metadata.csv"))

new_way_dt[,sessionTime := difftime(max(Datestamp), min(Datestamp), units = "mins"), by=SessionToken]
new_way_dt[,is_timeout := sessionTime >= 10 & sessionTime < 11, by=SessionToken]
new_way_dt[,Details := if_else(Details == "Session started" & is_timeout == TRUE , 
                                                                 "Session restarted after timeout", 
                                                                 Details), by=SessionToken]
        
setnames(new_way_dt, c("SessionToken","Datestamp","Details"),c("SessionID","Date","Event"))                                                
new_way_dt <- new_way_dt[,.SD, .SDcols=c("SessionID","Date","Event")]

new_way_dt[, c("WeekEnding","MonthEnding","EventCategory") := list(ceiling_date(Date, unit = "week", week_start = 1), 
...
new_way_dt <- new_way_dt[today() >= WeekEnding]
setDF(new_way_dt)
...
staging <- rbind(new_way_dt, old_way) %>%
...
all_week_deduped <- all_week %>% 
  filter(str_detect(Event, "User on tab") & !(Event %in% c('User on tabHome'))) %>%
...
counts_by_week %>%
  filter(str_detect(Event, "Unsuccessful upload")) %>%
...
counts_by_week %>%
  ...
  filter(!str_detect(Event, "Client Level Export")) %>% 
...
counts_by_week %>%
  filter(str_detect(Event, "Download")) %>%
  filter(!str_detect(Event, "DEMO MODE")) %>%
  filter(str_detect(Event, "System Overview") | str_detect(Event, "Client Level Export")) %>%
  ggplot(aes(x = WeekEnding, y = n, group = Event, color = Event, label = n)) +
  geom_line(size=0.75) +
  geom_point(size=1) +
  geom_text(nudge_x = .5, color = "gray15", check_overlap = TRUE) +
...
counts_by_week %>%
  filter(str_detect(Event, "Downloaded System Overview")) %>%
  filter(str_detect(Event, "DEMO MODE")) %>%
  ggplot(aes(x = WeekEnding, y = n, group = Event, color = Event, label = n)) +
  geom_line(size=0.8) +
  geom_point(size=1) +
...
```{r data by week}
gt(counts_by_week %>%
     filter(!str_detect(Event, "Unpacked file size")) %>%
     filter(!str_detect(Event, "Beginning upload")) %>%
     filter(!str_detect(Event, "Memory used after processing")) %>%
     select(
