...
# environment that you can find it again, navigate to the
# metadata-analysis/metadata directory in your Eva RStudio project, click
# Upload, then find the zip file you copied in. Click OK or whatever, then the
# files will be pasted in. Then run this.
...
#This is looking at metadata.csv, which does not have the SoftwareName column

new_way <-
  read_csv(here("metadata-analysis/metadata/metadata.csv")) %>%
...
old_way <- rbind(uploads_df, visits_df) %>%
  mutate(
    EventCategory = case_when(
      Event %in% c("Unsuccessful upload - not structurally valid",
      "Unsuccessful upload - not hashed",
      "Unsuccessful upload - wrong dataset",
      "Unsuccessful upload - wrong/incomplete dataset",
      "Unsuccessful upload - file was mistructured",
