...
get_syse_compare_time_chart <- function( isExport = FALSE){
  ...
  title <- paste0(title_start, 
                  c(paste0(': ', format(nrow(everyone() %>% fsubset(period == 'Current Year')), big.mark=',', scientific = FALSE)),
                    paste0(': ', format(nrow(everyone() %>% fsubset(period == 'Previous Year')), big.mark=',', scientific = FALSE))),
                  collapse='\n'
  )
