...
missing_client_location <- served_in_date_range %>%
  ...
  mutate(Type = "High Priority",
         ...
         Guidance = 
           str_squish("If Client Location is missing, this household will be excluded from
         all HUD reporting.")) %>%
  select(all_of(vars_we_want))
...
hh_no_hoh <- served_in_date_range %>%
  ...
  mutate(
    ...
    Type = "High Priority",
    Guidance = str_squish("Please be sure all members of the household are included in the 
